...
class Order extends BaseModel {
    ...
    protected $guarded = [];
    protected $appends = ['id', 'sn', 'total', 'payment', 'shipping', 'invoice', 'score','use_score', 'cashgift', 'consignee', 'status', 'created_at', 'updated_at', 'canceled_at', 'paied_at', 'shipping_at', 'finish_at','promos'];
        ...
        if (isset($status)) {
            switch ($status) {
                ...
                case self::STATUS_CANCELLED:
                    $model->where('order_status', self::OS_CANCELED);
                    break;
    ...
    public static function change_goods_storage($good_id, $product_id, $number = 0)
    {
        ...
        if (!empty($product_id))
        {
            ...
            if($number < 0){
                $products_query = Products::where('goods_id',$good_id)
                                        ->where('product_id',$product_id)->where('product_number','>=',abs($number))
                                        ->limit(1)
        ...
        if($number < 0){
            $query = Goods::where('goods_id',$good_id)->where('goods_number','>=',abs($number))
                      ->limit(1)
     ...
     *
     * @access  public
     * @param   json   $property       购物车对应的id json数组
     */
    public static function check(array $attributes)
    {
        ...
        if (($cart_id_count = count($cart_good_ids)) > 0) {
            $cart_data = Cart::whereIn('rec_id', $cart_good_ids)->get()->toArray();
        ...
        //查询默认收货信息
        $consignee_info = UserAddress::get_consignee(false);
        if(!empty($consignee_info)){
            unset($consignee_info['country']);
            unset($consignee_info['province']);
            unset($consignee_info['city']);
            unset($consignee_info['tel']);
        }

        //查询当前用户红包信息
        $bonus_info = UserBonus::getBonusByOrder($user_id,$totalMoney);

        //查询当前结算商品的信息
        // $goods_info = DB::table('cart')->select('goods_name as name', 'goods_price as price')->get();

        $result['consignee'] = $consignee_info;
        $result['bonus'] = $bonus_info;
    ...
    public static function closeOrder(){
        $setting =  config('app.close_order_time');
        $time = time() - $setting*60;
        $order = self::where('add_time','<=',$time)
                ->where('order_status',self::OS_UNCONFIRMED)
                ->whereIn('pay_status',[self::PS_UNPAYED, self::PS_PAYING])->get()->toArray();
        if(!empty($order)){
            $orderIds = array();
            ...
            unset($order);
            $orderGoodsInfo = DB::table('order_goods')->select('order_id','goods_id','product_id','goods_number')->whereIn('order_id',$orderIds)->get();
            $orderGoods = array();
            foreach ($orderGoodsInfo as $k => $v) {
                $orderGoods[$v->order_id] =$v;
            }
            unset($orderGoodsInfo);
            DB::beginTransaction();
            try {
                foreach ($orderIds as $key => $value) {
                    $returnBouns = self::return_user_integral_bonus($value);
                    $returnStorage = self::change_goods_storage($orderGoods[$value]->goods_id,$orderGoods[$value]->product_id,$orderGoods[$value]->goods_number);
                    if(!$returnStorage || !$returnBouns){
                        throw new Exception("退回库存和红包失败！");                      
                    }
                }
                $closeStatus = self::where('add_time','<=',$time)->whereIn('pay_status',[self::PS_UNPAYED, self::PS_PAYING])->update(['order_status'=>self::OS_CANCELED]);
                if(!$closeStatus){
    ...
    public static function closeTrade(){
        $setting =  config('app.close_trade_time');
        $time = time() - $setting * 24 * 60 * 60;
        return self::where('shipping_time','<=',$time)->whereIn('shipping_status',[self::SS_SHIPPED, self::SS_SHIPPED_PART, self::OS_SHIPPED_PART])->update(['shipping_status'=>self::SS_RECEIVED,'lastmodify'=>time()]);
    }
}
