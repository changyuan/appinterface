...
use App\Services\Payment\AlipayNew\AopClient;
use App\Services\Payment\AlipayNew\Request\AlipayTradeAppPayRequest;
...
class Payment extends BaseModel
{
    ...
    public static function pay(array $attributes)
    {
        ...
        $order = Order::where(['user_id' => $uid, 'order_id' => $order_id, 'pay_status' => Order::PS_UNPAYED])->with('goods')->first();
        ...
        if ($code == 'alipay.app') {
            ...
            $aop_client->appId              = $config['seller_id'];   //------------------------ 这里需要换成appid
            $aop_client->rsaPrivateKey      = $config['private_key'];
            $aop_client->format             = "json";
            $aop_client->charset            = "UTF-8";
            $aop_client->signType           = "RSA2";
            $aop_client->alipayrsaPublicKey = $config['public_key'];


            //实例化具体API对应的request类,类名称和接口名称对应,当前调用接口名称：alipay.trade.app.pay
            $alipay_req = new AlipayTradeAppPayRequest();
            $bizcontent = [
                "out_trade_no"    => $order->order_sn,
                "subject"         => $shop_name,
                "body"            => $shop_name,
                "timeout_express" => "30m",
                "total_amount"    => number_format($order->order_amount, 2, '.', ''),
                "product_code"    => 'QUICK_MSECURITY_PAY',
            ];
            $alipay_req->setNotifyUrl(url('/v2/order.notify.alipay.app'));
            $alipay_req->setBizContent(json_encode($bizcontent));
            $res_alipay_app = $aop_client->sdkExecute($alipay_req);
            //$data = htmlspecialchars($res_alipay_app);//就是orderString 可以直接给客户端请求，无需再做处理。
            return self::formatBody(['order' => $order, 'alipay' => ['order_string' => htmlspecialchars($res_alipay_app)]]);
        }
    ...
    public static function notify($code)
    {
        ...
        if ($code == 'alipay.app' || $code == 'alipay.app.old' || $code == 'alipay.wap') {
