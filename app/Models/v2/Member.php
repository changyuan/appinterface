...
use App\Services\Other\UserProxy;
...
class Member extends BaseModel {
    ...
    protected $guarded = [];
    protected $appends = ['id','rank','gender','username','nickname','mobile','avatar','points'];
    protected $visible = ['id','rank','gender','username','nickname','mobile','avatar','points'];

    public static function rsyncUserInfo($userid)
    {
        $user_id = DesHelper::des_decrypt($userid,"yaolan_app",true);

        if($user_id > 0){
            $userInfo = UserProxy::getInfoFormat($user_id,'UserBaseInfo,MarkInfo,UserExtInfo');
            $userData = $userInfo[$user_id];
            if(empty($userData['UserBaseInfo']) || empty($userData['MarkInfo']) || empty($userData['UserExtInfo'])){
                //未查到用户信息
                return self::formatError(self::BAD_REQUEST, trans('message.member.noinfo'));
            }else{
                //先同步用户信息，然后返回
                $shopUser['user_id'] = $user_id;
                $shopUser['user_name'] = $userData['UserBaseInfo']['UserName'];
                $shopUser['alias'] = isset($userData['UserBaseInfo']['NickName']) ? $userData['UserBaseInfo']['NickName'] : '';
                $shopUser['birthday'] = isset($userData['UserBaseInfo']['BirthDate']) ? $userData['UserBaseInfo']['BirthDate'] : '0000-00-00';
                $shopUser['sex'] = $userData['UserBaseInfo']['Gender'];
                $shopUser['email'] = isset($userData['UserExtInfo']['Email']) ? $userData['UserExtInfo']['Email'] : $user_id.'@yl.com';
                $shopUser['rank_points'] = isset($userData['MarkInfo']['TotalMark']) ? $userData['MarkInfo']['TotalMark'] : 0;
                $shopUser['reg_time'] = time();
                $shopUser['last_login'] = isset($userData['UserBaseInfo']['LastLoginDate']) ? substr($userData['UserBaseInfo']['LastLoginDate'],0,-4) : date('Y-m-d H:i:s');
                $shopUser['last_time'] = date('Y-m-d H:i:s');
                $shopUser['user_rank'] = isset($userData['UserBaseInfo']['NickName']) ? $userData['UserBaseInfo']['NickName'] : '';
                $shopUser['mobile_phone'] = isset($userData['UserExtInfo']['MobileTelephone']) ? $userData['UserExtInfo']['MobileTelephone'] : '';
                $shopUser['is_validated'] = ($userData['UserExtInfo']['MobileTelephoneIsBind'] == -1) ? 0 : 1;
                $shopUser['avatar'] = isset($userData['UserBaseInfo']['AvatarUrl']) ? $userData['UserBaseInfo']['AvatarUrl'] : '';
                $res = self::updateOrCreate(['user_id' => $user_id], $shopUser);
                $model = Member::where('user_id', $user_id)->first();
                if(!empty($model)){
                    $token = Token::encode(['uid' => $user_id,'timestamp'=>time()]);
                    return self::formatBody(['token' => $token, 'user' => $model->toArray()]);
                }else{
                    //同步用户信息出错
                    return self::formatError(self::BAD_REQUEST, trans('message.member.exception'));
                }                
            }
        }else{
            return self::formatError(self::BAD_REQUEST, trans('message.member.errorid'));
        }
    }
    ...
    {
        return $this->attributes['is_validated'];
    }
    ...
    {
        return $this->attributes['rank_points'];
    }
