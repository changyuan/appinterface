...
//        $products = json_decode($products, true);
    ...
    /**
     * 物流信息查询,一个订单可以拆分多个发货单，可能包含很多其他供应商的商品，所以有多个快递单号，这里区分对待
    ...
    public static function getDeliveyInfo(array $attributes)
    {
        ...
        if ($order = Order::where('order_id', $order_id)->where('user_id', $uid)->first()) {
            ...
            $info = DB::table('shipping')->where("enabled", 1)->select("shipping_id", "shipping_code")->get();
            $info = ArrayHelper::map($info, "shipping_id", "shipping_code");

            $delivery_info = DeliveryOrder::where("order_id", $order_id)->orderBy('update_time', 'desc')->get();
            if (empty($delivery_info)) {
                ...
                foreach ($delivery_info as $key => $value) {
                    $url = 'http://m.kuaidi100.com/query?type=' . $info[$value->shipping_id] . '&id=1&postid=' . $value->invoice_no . '&temp=' . $curr_time;
                    $res = file_get_contents($url);
                    $res = json_decode($res, true);
                    if ($res['status'] == 200) {
                        $temp = ['consignee_data' => $res['data'], 'consignee_name' => $value->shipping_name, 'consignee_no' => $value->invoice_no];
                    } else {
                        $temp = ['consignee_data' => [], 'consignee_name' => $value->shipping_name, 'consignee_no' => $value->invoice_no];
                    }
                }
                return self::formatBody($temp);
            }
            ...
            // $format_data = [];
            // }
            // return self::formatBody(['data' => $res['data'], 'vendor_name' => $order->shipping_name, 'code' => $order->invoice_no ]);
