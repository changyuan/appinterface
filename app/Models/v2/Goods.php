...
class Goods extends BaseModel
{
    ...
    protected $appends = [
        'id', 'category', 'brand', 'sku', 'default_photo', 'photos', 'name', 'price', 'current_price', 'discount', 'sales_count',
        'comment_count', 'is_liked', 'review_rate', 'intro_url', 'share_url', 'created_at', 'updated_at',
    ];

    protected $visible = [
        'id', 'category', 'brand', 'tags', 'videos', 'default_photo', 'photos', 'sku', 'name', 'price', 'current_price', 'discount', 'is_shipping', 'promos', 'stock', 'properties', 'sales_count', 'attachments', 'goods_desc', 'comments', 'comment_count', 'is_liked', 'review_rate', 'intro_url', 'share_url', 'created_at', 'updated_at', 'goods_name', 'goods_thumb', 'goods_img', 'is_on_sale', 'is_delete',
    ];
    ...
    public static function getList(array $attributes)
    {
        ...
        $data = $model
            ->with('properties')->with('tags')->with('videos')
            ->paginate($per_page)->toArray();
...
//        if ($data->promote_price == 0) {
    ...
    {
        return $this->hasMany('App\Models\v2\Tags', 'relation_id', 'goods_id');

    }

    public function videos()
    {
        return $this->hasMany('App\Models\v2\GoodsVideo', 'goods_id', 'goods_id');
