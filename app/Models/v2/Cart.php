...
class Cart extends BaseModel
{
    ...
    //购买数量，属性id，价格，库存（sku，或商品），属性值
    protected $appends = ['id', 'amount', 'property', 'price', 'attr_stock', 'attrs'];
    protected $visible = ['rec_id', 'amount', 'product', 'property', 'price', 'attr_stock', 'attrs', 'subtotal', 'goods_id'];
    /* 购物车商品类型 */
     ...
     *
     * @access  public
     * @param   integer $product($goods_id)   商品编号
     * @param   integer $amount        商品数量
     * @param   json   $property       规格值对应的id json数组
     * @param   integer $parent_goods_id     基本件
     * @return  boolean
     */
    public static function add(array $attributes)
    {
        ...
        $res = GoodsGroup::where('goods_id', $product)->where('goods_price', '<', $goods_price)->where('parent_id', $parent_goods_id)->orderBy('goods_price')->get(['parent_id', 'goods_price']);
    ...
    public static function checkout(array $attributes)
    {
        $user_id = Token::authorization();
        if($user_id <= 0){
            return self::formatError(self::BAD_REQUEST,trans('message.token.invalid'));
        }
        extract($attributes);
        //-- 完成所有订单操作，提交到数据库
        /* 取得购物类型 */
        ...
        if (($cart_id_count = count($cart_good_ids)) > 0) {
            $cart_data = self::whereIn('rec_id', $cart_good_ids)->get()->toArray();
            $cart_count = count($cart_data);
            if($cart_id_count != $cart_count)
            {
        ...
        foreach ($cart_goods_stock['goods_list'] as $value)
        {
            ...
            $cart_goods_info[] = ['id'=>$value['goods_id'],'price'=>$value['goods_price'],'parent_id'=>$value['parent_id'],'product_id'=>$value['product_id'],'goods_attr_id'=>$value['goods_attr_id']];
        }
        ...
        /*检查商品价格是否发生变化*/
        if(!self::change_goods_price($cart_goods_info)){
            return self::formatError(self::BAD_REQUEST,trans('message.good.price_change'));
        }
        unset($cart_goods_info);
       ...
       /* 判断是不是实体商品 */
        ...
        /* 支付方式 */
        if ($order['pay_id'] > 0) {
            $payment           = Pay::where('pay_id',$order['pay_id'])->first();
        ...
        try {
            ...
            foreach ($cart_goods as $key => $cart_good) {
                ...
                $orderRes = $order_good->save();
                if(!$orderRes){
                    throw new Exception(trans('message.cart.submit_error')); 
                    // DB::connection()->getPdo()->rollback();
                    // return self::formatError(self::BAD_REQUEST,trans('message.cart.submit_error'));
                }
            }
            ...
            if ($order['bonus_id'] > 0 && $temp_amout > 0) {
                $bonusRes = UserBonus::useBonus($order['bonus_id'], $new_order_id);
            }else{
                $bonusRes = true;
            }
            ...
            // if (ShopConfig::findByCode('') == '1' && ShopConfig::findByCode('stock_dec_time') == self::SDT_PLACE) {
            //     $reduceNumber = Order::change_order_goods_storage($order['order_id'], true, self::SDT_PLACE);
            // }else{
            //     $reduceNumber = true;
            // }
            $reduceNumber = Order::change_order_goods_storage($order['order_id'], true, self::SDT_PLACE);
            ...
            if($new_order_id && $orderRes && $bonusRes && $reduceNumber){
                DB::commit();
            }else{
                if(!$reduceNumber){
                    throw new Exception(trans('message.cart.out_storage'));                    
                    // return self::formatError(self::BAD_REQUEST,trans('message.cart.out_storage'));
                }else{
                    throw new Exception(trans('message.cart.submit_error')); 
                    // return self::formatError(self::BAD_REQUEST,trans('message.cart.submit_error'));
                }
            ...
            DB::rollback();
            return self::formatError(self::BAD_REQUEST,$e->getMessage());
        }
        ...
        unset($orderObj['score']);
        unset($orderObj['use_score']);
        unset($orderObj['promos']);
    ...
    {
        extract($attributes);
        Cart::where('rec_type', self::CART_GENERAL_GOODS)->where('rec_id', $rec_id)->delete();
    ...
    public static function updateAmount(array $attributes)
    {
        ...
        if (intval($use_storage) > 0 && $goods->extension_code != 'package_buy') {

            if (($goods->goods_number) < $amount) {
                return self::formatError(self::BAD_REQUEST, trans('message.good.out_storage'));
            }
            /* 是货品 */
            if (!empty($goods->product_id)) {
                $product_row = Products::where('goods_id', $goods->goods_id)->where('product_id', $goods->product_id)->first();
                if ($product_row->product_number < $amount) {
                    return self::formatError(self::BAD_REQUEST, trans('message.good.out_storage'));
                }
            }
        }

        Cart::where('rec_type', self::CART_GENERAL_GOODS)->where('rec_id', $rec_id)->update(['goods_number' => $amount]);
     ...
     * 是否删除，is_on_sale
    是否删除  is_delete
    检测商品库存数量是否够用 attr_stock
    客户端拿到这些数据做出判断是否可以继续购买
     * @return [type] [description]
     */
    public static function getList()
    {
        ...
        // ->where('goods.is_on_sale',1)
        // ->where('goods.is_delete', 0)
            ->select("cart.*","cart.goods_number as bug_num", "cart.goods_attr_id", "cart.product_id","goods.*")
            ->get();
            //这里cart被goods中的goods_number覆盖掉，所以重新as个bug_num

        $use_storage = ShopConfig::where('code', 'use_storage')->value('value');

        //$value->price // 计算过的现在的商品价格
        //$value->goods_price  // 之前的购物车价格
        foreach ($goods as $key => $value) {
            //订货数量大于0，在售卖中
            if ($value->bug_num > 0 && $value->is_on_sale == 1 && $value->is_delete === 0) {
                // $attr_id = empty($value->attrs) ? array() : explode(',', $value->attrs);
                // $goods_price = Goods::get_final_price($value->goods_id, $value->bug_num, true, $attr_id);
                
                //更新购物车中的商品价格
                self::where('rec_id', $value->rec_id)->update(['goods_price' => $value->price]);
            }
        }
     ...
     * @return  void
     */
    public static function change_goods_price($arr)
    {
        $goodsIds = array();
        $goodsArr = array();
        ...
        $goods = Goods::whereIn('goods_id',$goodsIds)->get()->toArray();

        foreach ($goods as $key => $value) {
            $goodsArr[$value['id']] = $value;
        }
        $goodsGroup = GoodsAttr::where('goods_id',80)->whereIn('goods_attr_id',explode(',', '19,21'))->get()->toArray();
        ...
        foreach ($arr as $key => $val) {
            ...
            $product_price = 0;
            if($val['product_id']){
                $product_info = GoodsAttr::where('goods_id',$val['id'])->whereIn('goods_attr_id',explode(',', $val['goods_attr_id']))->get()->toArray();
                if(!empty($product_info)){
                    foreach ($product_info as $k => $v) {
                        $product_price += floatval($v['attr_price']);
                    }
                }
            }
            if($val['parent_id'] > 0){//是配件，如果配件价格小于等于商品价格(促销时按促销价格计算)，应按照配件价格计算，否侧按照商品价格计算
                $goods_group_price = GoodsGroup::where('parent_id',$val['parent_id'])->where('goods_id',$val['id'])->value('goods_price');
                if($goods_group_price === null){//配件已不存在，直接返回
                    return false;
                }
                if($goods_group_price <= 0){//配件为免费赠送，则忽略不计
                    continue;
                }
                if($val['product_id'] && $product_price){
                    $now_price = ($goods_group_price <= $goodsArr[$val['id']]['current_price'] ? $goods_group_price : $goodsArr[$val['id']]['current_price']);
                    if(($now_price + $product_price) != $val['price']){
                        return false;
                    }
                }else{
                    $now_price = ($goods_group_price <= $goodsArr[$val['id']]['current_price'] ? $goods_group_price : $goodsArr[$val['id']]['current_price']);
                    if($now_price != $val['price']){
                        return false;
                    }
                }

            }else if($val['product_id'] && $product_price){//是货品，需要增加属性
                if($val['price'] != ($goodsArr[$val['id']]['current_price'] + $product_price)){
                    return false;
                }               
            }else{//是普通商品
                if($val['price'] != $goodsArr[$val['id']]['current_price']){
                    return false;
                }
            }
        }
