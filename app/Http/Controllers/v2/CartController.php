...
class CartController extends Controller {
    ...
    public function checkout(Request $request)
    {
        $rules = [
            ...
            "pay_id"           => "required|integer|in：1,2",          // 支付方式ID
            "invoice_type"     => "string|min:1",           // 发票类型ID，如：公司、个人
            "invoice_content"  => "required_with:invoice_type|string|min:1",           // 发票内容ID，如：办公用品、礼品
            "invoice_title"    => "required_with:invoice_content|string|min:1",           // 发票抬头，如：xx科技有限公司            
            "cashgift"         => "string|min:1",          // 红包ID
            "comment"          => "string|min:1",           // 留言
            // "score"            => "integer",                // 积分
            "cart_good_id"     => "required|json",         // 购物车id数组
        ];
